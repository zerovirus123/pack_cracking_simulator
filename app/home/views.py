...
def dropdown():
    request_session = requests_cache.CachedSession(
        "session_cache", expire_after=timedelta(days=1)
    )
    ...
    for set in set_json["data"]:
        if set["code"].lower() not in sets_to_ignore:
            if (
                ...
                or set["set_type"] == "draft_innovation"
                or set["set_type"] == "masters"
            ):
...
def get_set():
    if request.method == "POST":
        ...
        return render_template(
            "home.html",
            sets=sets,
            set_icon=set_json["icon_svg_uri"],
            image_uris=image_uris,
            selected_set="Select a Set",
        )
