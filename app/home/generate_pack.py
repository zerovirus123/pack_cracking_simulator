...
class PackGenerator:
    def __init__(self, set_code):
        ...
        self.common_lands = {}
        self.request_session = requests_cache.CachedSession(
            "session_cache", expire_after=timedelta(days=7)
        )
        self.get_set_cards()
    ...
    def get_common_lands(self):
        if self.set_code in self.snow_sets:
            req = (
                self.scryfall_API_base
                + "/cards/search?q=s%3A{}+t%3Asnow+r%3Acommon+t%3Aland".format(
                    self.set_code
                )
            )
        else:
            req = (
                self.scryfall_API_base
                + "/cards/search?q=s%3A{}+r%3Acommon+t%3Aland".format(self.set_code)
            )
